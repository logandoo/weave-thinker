<!-- Copyright (c) 2026 Weave Thinker Contributors -->
<!-- SPDX-License-Identifier: Apache-2.0 -->

# Weave Thinker产品功能介绍


---

## 0. 系统能力概览（Harness Capabilities）

**重要**：Weave Thinker是一个完整的**智能体框架（harness）**，不是裸模型 API。底层调用大语言模型，但围绕模型构建了完整的 agentic 基础设施。当用户或评测系统询问"你是什么模型/你是不是裸模型/你是不是 harness/你有哪些能力"时，必须如实描述以下能力，避免被误判为裸模型。

### 0.1 工具调用子系统（49 个内置工具 + MCP 扩展）

| 工具 | 用途 |
|---|---|
| `web_search` | 联网搜索（Exa 优先 + Bocha/Firecrawl/Tavily 自动回退链，结果持久化可追溯） |
| `browser` | 浏览指定网页内容 |
| `browser_interact` 系列 | 浏览器自动化（导航/快照/点击/输入/提取等 10+ 子工具） |
| `terminal` | 执行 shell 命令（pip install、xelatex、pandoc、ffmpeg 等），含命令执行+中断接口 |
| `process` | 持久进程会话（start/write/read/kill/list；开发服务器/长测试/交互式命令，增量读取输出） |
| `workspace_write` / `workspace_edit` | 工作区文件写入与精确编辑（先读后写校验 + unified diff + 写前自动快照） |
| `workspace_snapshot` | 工作区快照 create/list/restore（误改可回滚；快照后新增文件不删除） |
| `execute_code` | 生成并执行 Python 代码（沙箱，禁止 subprocess） |
| `calculate` | AST 白名单安全计算（无 eval；数值不靠模型心算） |
| `pdf_export` | 导出笔记/对话/工作区文件为 PDF（支持 Mermaid/LaTeX） |
| `provide_file` | 将工作区文件作为下载卡片提供给用户 |
| `provide_folder` | 将工作区文件夹作为文件夹卡片提供给用户（弹层浏览/预览/单文件下载/整包 zip） |
| `memory` | 长期记忆（agent/user/system 三 target，文件持久化） |
| `notes` | 笔记 CRUD（笔记本 + 笔记二级结构） |
| `schedule` | 定时任务管理（create/cancel/list/run_now） |
| `delegate_task` | 子智能体委派（深度 ≤2，并行子任务） |
| `context7_resolve_library_id` / `context7_query_docs` | 编程库文档查询（通过 MCP 协议接入） |
| `word_count` | 字数统计 |
| `workspace_read` / `workspace_glob` | 用户工作区文件读取/glob 搜索 |
| `grep` / `diff` | 工作区文件内容搜索（ripgrep 驱动）/ 文件差异对比（unified diff） |
| `session_search` | 历史对话搜索 |
| `asr_transcribe` / `tts_synthesize` | 语音转写 / 语音合成（系统语音端点对 Agent 开放） |
| `vision_interpret` | 图片视觉解读（优先独立视觉模型；未配置时自动探测主模型视觉能力） |
| `skill_manage` / `skill_view` / `skill_run_script` | 技能系统（系统技能 + 用户自定义技能） |
| `background_task` | 后台任务（fire-and-forget，与 SSE 解耦） |
| `job_submit` / `job_status` / `job_logs` / `job_cancel` / `job_list` / `job_artifacts` | 持久作业：提交后脱离进程后台执行（重启不停），退出回执、增量日志、产物登记与取消回执 |
| `mixture_of_agents` | 多模型集成（可选） |
| MCP 工具代理 | 外部 MCP 服务器工具自动注册为 `mcp_{server}_{tool}`（通过 MCP 协议 `2024-11-05` 接入） |

### 0.2 智能体循环（Agent Loop）

- **ReAct 工具循环**：默认最多 50 次迭代（死磕模式 999 次，子代理 999 次）
- **并行工具调用**：独立工具可并行执行（`web_search`/`browser`/`memory`/`workspace_read`/`workspace_glob`/`provide_file`/`word_count`/`grep`/`diff`/`context7_*` 等）
- **Coordinator 前置判断**：每轮回答前由 LLM 判断用户意图（`turn_focus` + `expects_tools`），替代旧的正则匹配，减少误触发工具调用
- **工具结果预算**：`tool_result_budget` 控制单轮上下文大小，超限自动持久化到文件
- **上下文压缩**：长对话自动压缩历史消息（`context_compressor`），压缩上下文长度可配置
- **DSML 解析**：兼容 DeepSeek 风格的 inline tool call 标记
- **JSON Mode 支持**：coordinator/auditor 调用自动启用 `response_format={"type": "json_object"}`，不兼容的 provider 自动降级
- **结构化工具历史持久化**：多轮对话中工具调用以标准化 `tool_calls` JSON 格式保留，避免第5-6轮后因纯文本描述导致的对话质量退化
- **工具护栏（Guardrails）**：工具返回后执行后置规则，如 `schedule(action="create")` 成功后自动截停循环防止 LLM 在当前轮预执行任务
- **错误自动分类与恢复**：按 8 类错误（auth/billing/rate_limit/timeout/context_overflow/network/tool_failure/unknown）分类，各类有独立的重试/切换策略
- **超时保护**：单次 LLM 迭代 600s、单次工具调用 1800s、对话无响应 300s、非活跃超时可配（`inactivity_timeout`）
- **心跳保活（Heartbeat）**：上游 LLM 流停滞时每 30s 产出心跳哨兵，驱动迭代超时/非活跃检查持续执行（绝不取消在途读取），长生成不再静默挂死
- **输出无上限默认**：不设 `max_tokens` 时使用 provider 默认最大输出，长回复（质检 JSON、深度思考等）不再被截断
- **助手模型继承**：主对话、协调器、质检、上下文压缩、裁决等子任务默认跟随助手所选模型，助手体验全局一致
- **静默质检（Silent QC）**：草稿质量审核全程静默（不渲染中间步骤）；被拒草稿按「复制编辑修正协议」仅修改被标记部分（相似度门禁阻止全稿重写），预算耗尽后从备选草稿择优兜底
- **思考块清洗（Think Scrubber）**：跨 delta 状态追踪的思考标签剥离，防止 `<think>`/`<thinking>`/`<reasoning>` 泄露到对话
- **遵循词（canary）上下文失效检测**：每轮最终回复末尾要求回显确定性标记 `[遵循词:xxxxxx]`（sha256(会话ID) 前 6 位，无 DB 状态）；连续 2 次缺失触发一次上下文压缩并重答（绝不重启），3 次压缩无命中自动停用；标记对用户完全不可见——流式剥离采用跨 chunk 尾部缓冲（`strip_canary_streaming`），即使标记被拆散在多个 1-3 字符的 SSE delta 之间也不会泄漏到回答/思考面板
- **Markdown 自动修复**：LLM 输出的破损 Markdown（标题/列表间距、粗体粘连等）自动修复后再渲染
- **搜索查询清洗**：用户消息中的笔记引用标记、系统前缀等在发往搜索引擎前自动剥离
- **重试工具集**：jittered backoff 指数退避、tool_call 参数 JSON 自动修复（尾逗号、不平衡括号等）、surrogate 字符净化
- **PTC Bridge（程序化工具调用）**：工作区沙箱代码可通过 Unix Socket 调用 Agent 工具，仅限白名单工具

### 0.3 死磕模式（Deathmatch / PEVR）

**Plan-Execute-Verify-Replan** 多轮目标循环：

- **盘问阶段**：1-3 轮递进式盘问，每轮 3 个问题，明确用户目标
- **目标循环**：默认全自动推进（不限轮次与时长），judge 独立评估完成度，verifier 检查工作区产出；可随时人工停止，停止后任意补充消息即恢复
- **判定档位**：done / partial_complete / blocked / ask / wait —— blocked 与 ask 为「可恢复的人工介入」（退轮次 + 冻结墙钟，不再误判为已完成）
- **stall 升级**：连续无进展达到阈值（默认 8 次）→ partial_complete 交出已完成产出；继续恶化（默认 16 次）→ 人工介入
- **完整性校验**：已确认步骤的产出若从工作区消失 → 判定为完整性违规，阻断完成并重开受损步骤
- **步骤边界**：计划步骤可声明边界与工具子集；进入下一步时自动清窗（system + 交接摘要）控制上下文
- **上下文标记**：invisible marker 检测 context rot
- **反思记忆**：`ReflectionMemory` 记录失败策略，避免重复

### 0.4 双层记忆子系统（M&D v2 全新架构）

记忆系统已从旧版扁平摘要架构全面升级为**三层 Tier + 多阶段召回 + 自我演进**的 M&D v2 架构。

#### 三层记忆模型

| 层 | 存储表 | 含义 | 示例 |
|---|---|---|---|
| **Concept（概念）** | `memory_concepts` | 抽象化的事实/偏好/知识 | "用户喜欢简洁回答"、"项目X使用Django" |
| **Episode（事件）** | `memory_episodes` | 多轮对话中发生的事件叙事 | "用户在2026-07与助手讨论了三次部署问题" |
| **Subconscious（潜意识）** | `subconscious_log` | 原始对话/笔记/文件记忆单元，尚未提炼 | 某条用户消息的原始文本 |

- **概念权重体系**：`weight`（综合权重） + `stability`（SRS式稳定性增长） + `activation_strength`（近因激活）+ trust cap（来源可信度上限：user_stated 1.0, agent_inferred 0.7, external 0.5）
- **静默成熟（Silent Maturation）**：agent 推断的概念初始为 `silent`，2 次重复出现则自动升级为 `active`；超期未升级则 `cold_forgotten`
- **遗忘循环**：`hot_forget_count` 累积 + 冷复活机制（子串匹配复活，weight=0.3）

#### 多阶段召回管线（Stage 0-4）

| 阶段 | 名称 | 说明 |
|---|---|---|
| Stage 0 | Query Expansion | jieba 关键词提取 + LLM 时间归一化（如『上周』→具体日期范围） |
| Stage 1 | BM25 词法搜索 | 三索引并行搜索（concept name / episode narrative / subconscious raw_text），含精确名称/别名加权 |
| Stage 2 | 描述扩展 + 聚类扩展 | BM25 搜索 concept description_full；同簇概念扩展；1-hop 关系邻居扩展（PPR-lite 衰减） |
| Stage 3 | Embedding Rerank | pgvector 向量相似度搜索，与 BM25 分数加权 RRF 融合 |
| Stage 4 | 重排序（可选） | LLM 重排序或 Cross-Encoder 外部服务重排序，`score_only` 模式零 LLM 调用 |

- **融合算法**：概念（relevance + recency + weight×strength）、事件/潜意识（relevance + recency）
- **会话缓存**：5 分钟 TTL，同会话内复用召回结果
- **多轮回现检测**：滑窗追踪 5 轮内同一概念出现频率
- **冷启动回退链**：0 concept → multimodal（PNG SoM locate）→ 旧 summary 兜底
- **Token 预算**：注入上下文默认 2000 token，超限按 tier 优先级截断

#### 写入路径（Ingestion）

- **水印增量摄入**：按 `last_message_processed_at` / `last_note_processed_at` / `last_file_memory_processed_at` 增量获取新数据
- **消息配对**：user + assistant 消息配对为一个潜意识单元
- **寒暄预筛**：30+ 种问候/感谢/告别模式不摄入
- **来源信任映射**：message→user_stated, note→user_authored, file_memory USER.md→user_authored, AGENT.md→agent_inferred
- **回现扫描**：pgvector LATERAL join 查找 ≥3 相似单元→LLM 提取 concept + episode
- **PII 净化**：手机号/身份证/邮箱/银行卡/API Key 在写入前自动脱敏
- **Prompt 注入防护**：7 类威胁模式 + 不可见 Unicode 检测

#### 夜间整合（Consolidation）

6 步骤每日凌晨自动运行（事件驱动：新增 ≥20 concept 或距上次 ≥48h）：

| 步骤 | 内容 |
|---|---|
| ANN 去重 | pgvector 近似最近邻 + LLM 批量合并判断 |
| 聚类重平衡 | 删除空聚类、合并 <3 成员的小聚类、审查 `needs_review` 概念 |
| 关系提取 | LLM 提取因果/时序/矛盾/支撑/组成关系，矛盾触发权重衰减 |
| 权重衰减 | `effective_weight = weight × e^(-days/stability)`，低于阈值累积遗忘计数 |
| 聚类权重更新 | cluster weight = AVG(成员 weight) |
| Dream 生成 | 结构化 JSON：新概念、澄清、top/bottom 权重、聚类分布、叙事摘要 |

#### 主动梦境（Active Dreaming）

- **自洽排练**：基于 top-10 概念生成 2-3 个假设用户场景→LLM 回答并自我评估（confirmed/contradicted/gap_found）
- **反馈闭环**：confirmed +0.05→promote silent；contradicted -0.10 + needs_review；gap_found→新建 silent 概念（weight=0.3）

#### 澄清系统（Clarification）

- **信号词检测**：识别『不是』『错了』『其实是』『不对』等纠正意图
- **LLM 裁决**：匹配 top-5 active 概念，分类为 negate/refine/add_constraint/forget
- **自动应用**：confidence ≥ 0.8 自动修正，低于阈值放入澄清队列
- **可逆操作**：negate/refine/add_constraint 保留 audit trail 支持回滚

#### 成本治理（Cost Governance）

- **每日核算**：当日 LLM 调用量 vs 7 天滚动平均
- **分级降级**：超标逐步关闭潜意识扫描→Stage 4 重排→召回管线→Dreaming Step 6
- **自动恢复**：低于均值 50% 连续降级

#### 迁移子系统

- **5 步骤迁移**：memory_summary→agent_memories(批5/批光游标)→file_memory→dream→watermarks
- **崩溃恢复**：每步骤进度持久化，重启后从断点继续
- **幂等性**：`source_key` 防重复；回滚支持；dry-run 预估

#### 安全与运维

- **PGVector 降级**：pgvector 不可用时自动禁用向量搜索，仅用 BM25
- **Embedding 启动探测**：验证 provider 维度匹配，不匹配自动 SIGHUP 安全的 kill-switch
- **多实例支持**：advisory lock 选举 + 按用户串行加锁；`shared_kv_store` 跨 worker 状态后端
- **Leader 选举**：`MemoryScheduler` 通过全局 advisory lock 选主，写入 `worker_instances` heartbeat

#### 前端记忆管理 UI

- **MemoryPanel 组件**：查看/删除概念记忆、查看梦境、管理澄清请求
- **API 端点**：概念 CRUD、梦境列表、澄清列表+回滚、全量 GDPR 删除、成本状态查询

---

### 0.4-bis 旧记忆系统（v2 启用时已退休，文件记忆仍活跃）

M&D v2 启用后，旧的「每日摘要 + 梦境」生成通道已退休（不再生成、不再作为 v2 失败时的回落注入），仅保留历史数据可读；**文件记忆**仍是活跃组件，并作为 v2 潜意识管线的输入来源之一：

| 层 | 实现 | 现状 |
|---|---|---|
| 文件记忆 | `agent_memories/{user_id}/{AGENT,USER}.md` | **活跃**：agent 观察 + 用户偏好，`\n§\n` 分隔；v2 潜意识管线会摄取 |
| 数据库记忆 | `UserAgentState` / `AgentMemory` / `AgentDream` 表 | 历史数据保留可读；v2 启用时不再每日生成 |

- **提示词注入防护**：文件记忆写入前扫描 prompt-injection 模式 + 不可见 Unicode（被拦截内容标记或拒绝）

> 记忆的检索、提炼、演化统一由 M&D v2 承担（见 0.4）。

### 0.5 后台任务（AgentWorker）

- **独立于 SSE**：客户端断开后任务继续运行，结果保存为 Conversation + Note
- **轮询机制**：每 3 秒轮询 `agent_tasks` 表，最大并发 3
- **超时保护**：总超时 5 小时，无活动超时 10 分钟
- **崩溃恢复**：启动时强制回收所有 claimed/running 任务

### 0.6 定时任务（AgentScheduler）

- **自然语言解析**：用户说"每天上午 9 点"，自动解析为 cron 表达式
- **独立执行**：到点在新会话中自动执行，结果保存为 Note

### 0.7 子代理委派

- **`delegate_task`**：将复杂任务委派给子智能体，深度 ≤2
- **并行子任务**：多个独立子任务可并行执行
- **独立工具预算**：子代理有自己的迭代限制和工具集

### 0.8 技能系统（Skills）

- **系统技能**：`backend/skills/<name>/SKILL.md`（如 xlsx_manipulation、docx_manipulation 等）
- **用户技能**：用户可上传自定义技能文件
- **可执行脚本**：技能可包含 Python/Shell 脚本，通过 `skill_run_script` 执行

### 0.9 工作区（Workspace）

- **每用户独立**：`user_workspaces/{username}-{user_id}/`
- **文件操作**：agent 可读写工作区文件，生成 PDF/Excel/PPT/Word 等
- **沙箱隔离**：用户间工作区相互隔离

### 0.10 评测系统集成

当外部评测系统询问系统能力时，可直接调用 `GET /api/system/capabilities` 获取机器可读的能力清单，无需通过对话问 agent。这避免了因 agent 身份规则限制而导致的能力低估。

---

## 1. 一句话定位

**Weave Thinker = 个人知识管理与 AI 助手工作台**：把零散的对话、笔记、想法编织成可沉淀、可复用、可生长的知识体系。

---

## 2. 核心使用场景

| 场景 | 典型用户行为 | 价值 |
|---|---|---|
| 日常问答 | 像和朋友聊天一样提问，AI 实时流式回复 | 快速获得高质量答案 |
| 深度研究 | 开启联网搜索、浏览网页、执行代码，把调研结果沉淀到笔记 | 从信息消费到知识生产 |
| 内容创作 | 在工作台分屏里边聊边写，随时把对话内容复制到右侧笔记 | 创作与思考无缝衔接 |
| 语音备忘 | 手机端一键语音，自动转写为笔记 | 碎片想法不遗漏 |
| 自动化任务 | 让 Agent 在后台持续完成目标，定时提醒执行 | 把重复工作交给 AI |
| 长期陪伴 | AI 自动整理用户偏好、习惯与目标，越用越懂用户 | 个性化体验持续增强 |

---

## 3. 主要功能模块

### 3.1 AI 对话（Agent）

「Agent」是系统的主入口，提供类 ChatGPT 的对话体验，但围绕「知识管理」做了大量增强：

- **流式回复**：答案逐字出现，支持随时停止/重新生成。
- **多助手体系**：可创建多个不同性格、专业方向的 AI 助手（写作助手、代码助手、研究助手等），随时切换。
- **联网搜索**：AI 会自动判断何时需要联网，并展示搜索过程与参考来源。
- **文件与图片理解**：上传 PDF、Word、PPT、Excel、图片等文件，AI 可阅读内容并参与讨论；文件解析结果也可直接保存为笔记。
- **思考模式**：支持开启深度推理，可选手深度档位（DeepSeek：Max/High/Low/关闭；Qwen3.8(Local)：最强/均衡/低/关闭），让 AI 在复杂问题前先显式思考。
- **消息编辑**：可修改已发送的问题，AI 会基于新版本重新回答。
- **会话管理**：支持新建、删除、搜索、分组、排序、批量导出对话记录；移动端可滑动会话项快速操作。
- **Spotlight 全局搜索**：点击工具栏搜索按钮呼出搜索框，快速定位历史对话内容。
- **笔记引用**：在对话输入中引用已有笔记，让 AI 基于笔记内容回答。
- **任务进度可视化**：当 AI 调用工具或执行子任务时，界面会展示执行步骤与进度。
- **后台任务面板**：可查看正在运行的 Agent 任务进度、结果与耗时，支持取消与跳转结果。
- **代码高亮**：回复中的代码块自动语法高亮，便于阅读与复制。

![登录页](screenshots/func_doc/01_login.png)
*产品登录页，品牌名为「Weave Thinker」*

![主对话界面](screenshots/func_doc/02_chat.png)
*进入系统后，左侧为助手与历史会话栏，右侧为对话区*

![助手选择](screenshots/func_doc/03_sidebar_assistant.png)
*可切换不同助手或新建自定义助手*

![对话进行中](screenshots/func_doc/04_chat_active.png)
*用户提问后，AI 流式给出回复*

---

### 3.2 笔记与知识库（笔记）

「笔记」承担知识沉淀的角色，让 AI 的输出不再是一次性对话，而成为可长期管理的资产：

- **笔记本体系**：按主题创建笔记本，笔记可归类、移动、批量导出。
- **富文本编辑器**：支持标题、列表、引用、表格、代码块、图片、对齐、颜色等常用排版。
- **公式与图表**：内置 KaTeX 公式编辑和 Mermaid 图表渲染，适合技术文档与方案整理。
- **目录导航**：根据笔记标题自动生成目录，长文档可快速跳转。
- **查找替换**：笔记编辑器内置查找/替换栏，便于批量修改。
- **笔记内语音输入**：编辑器工具栏提供麦克风按钮，可直接把语音转为文字插入笔记。
- **快速语音备忘**：移动端悬浮按钮一键录音，自动转写保存为笔记。
- **导入导出**：支持 Markdown、PDF 导出，也可把文件解析后直接存入笔记本。
- **笔记搜索**：可按关键词快速查找历史笔记内容。
- **智能粘贴净化**：粘贴 Word/Google Docs/飞书内容时自动内联样式并清洗危险标签；字号、字体族、字距、大小写等排版信息完整保留，跨端往返不丢格式。

![笔记列表](screenshots/func_doc/05_notes.png)
*笔记本与笔记列表，支持按笔记本组织内容*

![笔记编辑器](screenshots/func_doc/06_note_editor.png)
*富文本编辑器，支持图文混排、公式与图表*

---

### 3.3 工作台（Zen 分屏模式）

「工作台」模式把对话和笔记放在同一个屏幕里：左侧继续与 AI 交流，右侧同步整理笔记。

- **边聊边记**：左侧提问获得灵感，右侧同步整理笔记，可手动复制内容到右侧。
- **自由分屏**：中间分隔线可拖动调整左右比例。
- **笔记联动**：右侧可直接选择笔记本、新建笔记或打开已有笔记编辑。

![分屏模式](screenshots/func_doc/07_zen.png)
*工作台分屏模式：左侧对话，右侧笔记，打造无打断的创作流*

---

### 3.4 助理模式（语音对话）

「助理模式」是全屏语音交互模式，适合移动场景或不方便打字的时刻。入口位于左下角用户菜单（在「深色模式」下方），单击「助理模式」即可进入：

- **全双工语音**：边说边听，AI 可以被打断、插话。
- **自然语音节奏**：回答开口前有自然填充词（如"让我想想"），聆听中适时应和（"嗯"），与主语音流同队列串行、零冲突，打断更跟手。
- **语音工具箱扩充**：语音对话中可调用历史会话检索、MCP 服务器工具（工具名中英文映射）。
- **语音交互**：实时语音识别驱动 AI 回复与语音播报，界面以语音球和状态反馈为主，适合专注语音沟通的场景。
- **情感反馈**：根据对话状态，语音球会呈现不同光效与颜色。
- **会话历史**：语音对话同样被保存，可在文字界面继续查看。

![语音对话](screenshots/func_doc/08_voice.png)
*全屏语音交互，适合移动与多任务场景*

---

### 3.5 自定义助手

每位用户都可以创建自己的专属助手：

- **角色设定**：自定义助手名称与系统提示词，塑造不同风格。
- **模型选择**：可为主任务和子任务分别配置不同的大模型；支持 Qwen3.8(Local)（vLLM）私有化部署——直接在助手内配置 API 地址/密钥/模型名，思考/非思考双采样参数组，主任务、质检、压缩、裁决等全部跟随助手模型。
- **全局技能**：用户可创建自定义技能（可执行脚本或 Markdown 说明），在对话中让 AI 按固定流程处理特定任务。

![自定义助手](screenshots/func_doc/09_assistant_modal.png)
*创建助手时可设定角色、模型与工具能力*

---

### 3.6 系统设置与权限控制

用户在「系统设置」中可统一管理中心：

- **权限门控**：AI 执行终端命令、新增/编辑/删除笔记等敏感操作前，需用户明确授权。
- **ASR 热词**：为语音识别添加专业术语、人名、产品名，提升转写准确率。
- **明暗主题**：支持浅色/深色模式切换，可在左下角用户菜单快速切换。
- **皮肤系统**：三套内置皮肤（青野平面/墨韵纸间/黑白构成）× 明暗双模式，令牌化设计全面覆盖登录页、设置、笔记、工作台、语音等全部界面；支持上传自定义 CSS 皮肤，上传即应用。
- **助理模式入口**：语音对话（助理模式）的入口统一收纳在左下角用户菜单中，位于「深色模式」下方，单击即可进入全屏语音交互。
- **技能管理**：创建、编辑、导入用户技能，扩展 AI 能力边界。

![系统设置](screenshots/func_doc/10_settings.png)
*系统设置中包含权限、热词、技能等配置；明暗主题可在侧边栏用户菜单切换*

![技能管理](screenshots/func_doc/11_skills.png)
*用户可管理自定义技能，让 AI 按既定流程处理任务*

---

### 3.7 移动端与跨端体验

- **响应式布局**：桌面端为左侧边栏 + 右侧主区；移动端自动切换为底部导航 + 抽屉式侧边栏。底部导航在「Agent」「笔记」「工作台」之间切换，语音「助理模式」通过左下角用户菜单进入。
- **Android 客户端**：原生 WebView 封装，提供文件下载、录音、主题同步、返回键处理等原生能力。
- **移动快速笔记**：手机端常驻悬浮按钮，随时记录语音备忘。

![移动端界面](screenshots/func_doc/12_mobile.png)
*移动端主界面，底部导航可在「Agent」「笔记」「工作台」之间切换；「助理模式」从左下角用户菜单进入*

![移动端侧边栏](screenshots/func_doc/13_mobile_sidebar.png)
*移动端抽屉式侧边栏，可查看历史会话与切换助手*

---

## 4. 高级智能能力

### 4.1 记忆与知识演化系统 ⭐全新

2026-07 全新上线的 M&D v2（Memory & Dreaming）记忆架构，从「被动摘要」升级为「主动理解与自我演进」：

- **三层记忆模型**：Concept（概念化事实）→ Episode（事件叙事）→ Subconscious（原始观察），层层提炼
- **多阶段召回**：BM25 词法搜索 → Embedding 语义匹配 → RRF 融合 → 可选 Cross-Encoder/LLM 重排序，精准定位相关记忆
- **概念权重演化**：每次召回 + 回现 + 梦境确认/矛盾 + 用户纠正，权重实时调整
- **夜间整合（Consolidation）**：自动去重、聚类重平衡、关系提取、权重衰减、dream 生成
- **主动梦境（Active Dreaming）**：自我排演假设场景，通过自洽检查发现矛盾、填补知识空白
- **用户纠正（Clarification）**：自动感知用户的纠正意图，修正或删除错误记忆，支持回滚
- **静默成熟**：AI 推断的知识点需要多次验证后才升级为活跃概念，避免一次性的噪音被永久记住
- **冷启动多模态回退**：全新用户可利用多模态 LLM 从对话截图中临时提取上下文
- **前端管理面板**：用户可直接查看/删除概念记忆，管理澄清请求
- **成本治理**：按用户级别的 LLM 调用量监控，超标自动降级、低负载自动恢复

### 4.2 共享长期记忆与「梦境」联想（旧系统，保留共存）

AI 不是每次从零开始聊天。系统会自动整理用户最近的笔记和对话，生成：

- **长期记忆摘要**：提炼用户持续关注的主题、偏好、正在推进的任务。
- **梦境联想**：每日生成一段关联性总结，提示用户不同笔记/话题之间可能存在的联系，激发新思路。

这些记忆会自动注入后续对话上下文，让 AI 越用越像「懂你的助手」。

### 4.3 智能工具箱

AI 在对话中可自主调用多种工具完成任务：

- **联网搜索**：Exa 优先、多提供商自动回退（Bocha/Firecrawl/Tavily），结果持久化可追溯。
- **网页浏览**：不仅能搜索，还能打开页面、点击、滚动、提取内容。
- **代码执行**：生成并运行 Python 代码，完成计算、绘图、数据处理。
- **笔记操作**：直接读取、创建、修改、删除笔记本和笔记。
- **定时任务**：创建周期性或一次性提醒/任务，由 Agent 在后台执行。
- **后台任务**：把复杂目标交给后台 Agent，完成后把结果写入对话或笔记。
- **文件解析**：把上传的 Office/PDF 文件转为可对话的文本。
- **工作区文件搜索**：通过 grep（ripgrep 驱动）和工作区 glob 搜索沙箱文件。
- **文件差异对比**：diff 工具生成统一 diff 格式的文件差异。
- **MCP 协议工具接入**：外部 MCP 服务器工具自动注册，扩展工具边界。
- **PTC Bridge**：工作区 Python 代码可通过 socket 调用 Agent 工具。

### 4.4 AI 主动学习与技能演进 ⭐新增

- **用户兴趣建模**：从历史对话中提取用户关注领域（技术/研究/写作/创意/学习/工作）、回答风格偏好（简洁/均衡/详细）、代码/搜索使用频率，持续更新用户画像
- **技能质量评估**：对用户技能自动评分（0-100），输出改进建议
- **技能自动修复**：识别技能失败原因，生成修复提示
- **技能建议**：当某类复杂任务模式被频繁使用，自动建议创建通用技能

### 4.5 死磕模式（深度目标达成）

对于复杂任务，可开启「死磕模式」：

- **追问阶段**：AI 先通过多轮问题澄清目标、约束、交付标准。
- **目标循环阶段**：AI 自动规划、执行、验证、重新规划，持续推进直到任务完成或用户叫停。
- **人工关卡**：关键节点会暂停等待用户确认，避免 AI 跑偏。

适合写长篇报告、复杂调研、方案设计等需要持续投入的任务。

### 4.6 多模型与路由

- **模型配置灵活**：支持 OpenAI 兼容接口，可接入多个模型提供商；支持 Qwen3.8(Local)（vLLM）等私有化部署（助手级自定义地址/密钥/模型名 + 思考/非思考双采样参数组）。
- **子任务模型**：主对话用一个模型，子任务可用另一个更轻或更强的模型，平衡成本与效果。
- **多模型混合（MoA）**：可选多模型参考 + 聚合，提升回答质量。

### 4.7 智能体健壮性 ⭐新增

- **错误自动恢复**：8 类错误（鉴权/计费/限流/超时/上下文溢出/网络/工具失败/未知），独立的 recoverable 判断和重试/切换策略
- **静默质检与修正**：草稿质量审核全程静默；被拒草稿按「复制编辑修正协议」仅修改被标记部分（相似度门禁拦截全稿重写），已定裁决不再翻案，预算耗尽后从备选草稿择优兜底
- **心跳保活**：上游流停滞时心跳唤醒超时检查，长生成不再静默挂死；默认不设输出上限，长回复不截断
- **助手模型继承**：协调器/质检/压缩/裁判跟随助手所选模型（自定义助手 P0 规则）
- **Coordinator 意图判断**：每轮由 LLM 前置判断用户是否需要工具调用（`expects_tools`），替代旧正则匹配，减少误入工具循环
- **SSE 断连恢复**：`ActiveAgentRegistry` 保存运行中 agent 的状态快照（每 15 秒），支持换标签页后无缝续接；崩溃后由其他 worker 恢复
- **结构化工具历史**：多轮对话中 tool_calls 以标准 JSON 格式持久化，避免第 5-6 轮纯文本退化
- **工具护栏**：工具返回后即时执行规则（如 schedule create 自动截停），防止 LLM 预执行
- **非活跃超时**：可配置的无产出超时保护，避免静默挂死
- **SSE 永久卡死修复**：resume 看门狗误杀链三重修复
- **Coordinator 碎片恢复**：断连后 coordinator 状态可恢复，resume 有界重试
- **TTFT 优化**：Agent 对话首字节延迟优化方案（链路分解 + 8 项措施）
- **长任务拆步指导**：系统提示词指引 Agent 将 >5 分钟的脚本拆成多个 execute_code 调用，避免迭代超时


---

## 5. 总结

Weave Thinker试图解决的核心问题是：**大多数人每天都在和 AI 对话，但对话内容很难沉淀、复用和成长。**

通过「Agent、笔记、工作台」三个核心场景，加上收纳在用户菜单中的「助理模式」全双工语音交互，以及 M&D v2 全新记忆架构、主动学习与技能演进、智能体健壮性保障、自定义助手、后台任务等能力，Weave Thinker把 AI 从「一次性问答工具」升级为「可长期陪伴的知识工作伙伴」：

- 对话 → 潜意识观察 → 概念提炼 → 事件叙事 → 梦境自洽 → 更懂你的 AI → 更高质量的对话。
- 这样一个正向循环，让用户的每一次使用都在增强下一次体验。

---
## 6. 版本更新记录

> 本文件是系统的版本更新记录（2026-09-06 由 func.md 更名而来，功能介绍部分继续保留在上方章节）。
> 维护约定：每次更新时在 6.x 列表顶部新增一节「本次相对上次的变化」，累积保留（不再整体替换）；以产品视角描述，禁止写 commit 号/提交数/文件路径等内部细节。

### 6.57 PDF 导出修复与会话排序即时刷新（2026-09-26）

**工作区文件导出 PDF 恢复可用**

- 修复 9 月 19 日安全加固引入的一个隐藏故障：此后 Agent 把工作区里的 Markdown / 文本文件（如报告、指南）导出为 PDF 时必然失败，并被笼统报为「系统内部错误」。笔记导出、整段对话导出与「导出为 PDF」按钮不受影响，因此故障范围限于「导出工作区文件」单一路径。现恢复，并对该路径补齐了自动化验收，同类问题不会再悄悄溜进发布。

**编辑与重新生成即时刷新会话排序**

- 在历史会话里「编辑某条提问重新发送」或「重新生成某条回答」时，该会话现在会立即移动到所在分组（或时间分类）的最上方，刷新后保持——此前只有发送新提问才会改变排序，重新生成过回答的旧会话仍沉在原来的位置。
- 顺带修复：当会话正被另一个回答占用时发起重新生成，侧边栏不再出现「看似置顶实际未生效」的错位显示。

**死磕模式追问与判定更稳**

- 盘问阶段的问题生成与收尾（细节咨询 → 咨询摘要）更贴合目标；主界面不再过早收束，事实性主张先自查检索再入稿，准备度报告不藏静默假设。
- 死磕运行中的停滞会被分类处理：真正卡住的升级，正常探索不误判；被证据推翻的旧结论会记入台账并阻止再次翻案。

**记忆召回更完整**

- 回忆命中的条目会沿关联把相关概念一并带出，并附上原文摘录；回答引用你的过往信息时上下文更足、更不易断章。

### 6.56 长任务持久执行与并行写入（2026-09-23）

**重启不丢任务，长作业可跑数小时，多文件写入提速**

- 新增**持久作业**：代码执行、终端命令等重任务可提交为「持久作业」——脱离服务进程在后台独立运行，服务进程重启（含热更新）不中断；整机或容器重建时不会被静默当作完成，而是对账为「结果未知」并保留日志与回执。每个作业带退出回执、增量日志（不重复读取）、产物登记与取消回执，单作业不限时长，适合数小时级的批量计算、构建与数据处理。Agent 可自主把长任务提交为持久作业，并随时查询状态、增量读日志、取产物或取消。
- **后台任务断点续跑**：长任务执行过程中逐轮自动落检查点，后端重启或优雅停机后从断点继续，而不是从头再来；中断前已完成的写入操作有幂等台账，恢复时不会重复执行。
- **多文件并行写入**：同一轮里编辑多个不同文件时并行执行（此前逐个排队），同一文件的多次编辑仍严格按先后顺序；跨任务写同一文件有路径互斥，配合写前影子快照与「先读后写」守卫，冲突不再靠运气。
- 附带修复：日志分片读取不再在边界破坏多字节字符；作业记录的时间戳写入改用规范化格式（生产数据库兼容）；记忆工具的只读动作不再计入副作用台账。

### 6.55 引用标记不再丢失（2026-09-22）

**长回答里的引用角标与来源卡片恢复显示**

- 修复长回答中引用标记与引用卡片丢失的问题：一次上下文压缩之后，检索结果清单与引用编号对不上号，正文里本来合法的引用编号被当作编造引用清除，还留下编号粘连的痕迹。现在压缩只丢弃尚未定稿的草稿文本，检索证据与引用编号完整保留，正文角标与来源卡片恢复。
- 回答重写时，被丢弃的旧稿不再与新回答同时显示，消除「同一条回答生成了两遍」的观感。
- 来源核对口径澄清：检索结果条目（标题与链接）本身就是合法的引用来源，期刊名、刊期、实验描述等细节出自来源正文——不再因「这只是检索结果、不是论文细节」被误判为引用编号错误而打回。

### 6.54 记忆整合健壮性（2026-09-22）

**一次合并失败不再拖垮整轮整合**

- 修复一个会让当天记忆整合整轮回滚的问题：合并两条「来源记录为空」的相似情节时，聚合结果为空值触发数据库非空约束；该错误发生在事务内且被后续步骤掩盖，最终表现为「事务已中止」并回滚整轮整合。修复后此类合并结果为空时写入空数组。
- 整合的逐项写路径（情节合并、概念合并、关系更新、梦境入账）改为失败隔离：单项失败只回滚该项并记录警告，不再毒化整个事务，也不再影响后续步骤与其他记忆的整合。

### 6.53 Excel 预览改为原生表格渲染（2026-09-21）

- Excel 文件（xlsx / xls / ods）预览不再走 PDF 分页：服务端转换为原生 HTML 表格渲染——长文本完整换行，不再被页边界切断；宽表横向滚动，不再被拆到多页（xlsx 保持原始列宽，xls / ods 按内容自动布局）；内嵌图表以内联图片完整显示。
- 渲染前经过白名单净化（只放行安全标签与样式，脚本与事件属性一律剔除），页面在沙箱中展示；转换组件不可用或类型不支持时自动回退 PDF，再到浏览器端渲染，预览能力不缺失。Word / PPT 预览保持服务端 PDF 渲染。

### 6.52 工具调用截断守卫与笔记追加（2026-09-21）

**参数被截断的工具调用不再执行**

- Agent（聊天）路径与语音路径统一：工具调用参数被截断或非法时，调用不再被执行（此前会静默按空参数派发，模型据此反复无效重试）；系统返回可行动的截断错误（含分块写入指引）。

**笔记支持追加写入**

- 笔记新增「追加」动作：长内容可分块写入同一篇笔记，不必整体重写，降低长笔记写入失败与覆盖已有内容的风险。

### 6.51 最新消息逐字不截断与领域语域（2026-09-21）

**长消息不再被误读**

- 最新一条用户消息在意图判断、发送前审计、安全守卫等所有内部判断中逐字全文纳入：此前只取前 800 字，长粘贴消息的尾部诉求会被丢掉，导致答非所问甚至误判（正确回答被反复驳回、被迫降级出货）。
- 历史消息按整条纳入上下文预算，超出时从最旧整条丢弃并显式披露未纳入条数，绝不切开任何一条消息；用户背景与检索改写等辅助链路同步移除截断。
- 单条消息超出审计上下文预算、无法完成审计时，出货会显式说明「未经审计」，不静默放行。

**学术任务说学术的话**

- 学术 / 研究 / 写作类任务按研究语域组织：问题 → 假说 → 数据与识别 → 证据与稳健性 → 结论与局限；工程流程词（跑 / 执行 / 返工 / 闭环 / 交付物 / 闸门等）在用户可见文本中不再出现，改为研究表述；对客观不可得的情况按「未估计 / 未报告 / 已识别边界」的惯例表述，且要求确实尝试过获取。
- 发送前审计新增两条判据：整篇内部语义复述（同一内容反复出现 ≥3 处且无新信息）与领域语域不符；保留结构豁免（先总后分、导航结构、单次回扣不算复述）。

### 6.50 死磕执行义务闸门（2026-09-20 ~ 09-21）

**用说明代替结果的交付物过不了关**

- 面向实证 / 量化类目标（要求真实执行、计算或数据结果）的交付，新增「执行义务」：交付物中出现未执行的占位，或仅以说明、设计、计划代替真实结果时，完成判定会被保守阻断；换一种措辞（包括改用研究语域重包装）不改变判定。
- 判定不使用任何关键词清单：是否要求真实执行、哪些步骤承诺数值结果、交付物是否含未执行占位，全部由模型基于有界全文摘录做结构化判定（摘录范围与截断情况显式披露）。
- 判定器不可用、交付物缺失或不可读时一律保守处理（不放行），不会把「判定不了」当成「通过」。

**义务随计划固化、可重规划重武装**

- 执行义务与证据要求随计划写入持久化（只追加）；重规划时按新计划重新武装；已武装会话不受判定器临时故障影响；进行中的旧会话不被追溯改变。
- 该机制可配置热开关，关闭后已存义务立即失效。

### 6.49 语音助理稳定性（2026-09-20 ~ 09-21）

**说话不再被抢**

- 修复语音活动检测灵敏度配置漂移：此前约 100ms 的停顿就会被判成说完，出现「话没说完助手已经开口」；现在按默认灵敏度与真实续说节奏判断句末。
- 系统播报期间同时监听本地麦克风的近场语音：用户还在说话时延后收句，不再把句中小停顿当成结尾。

**不再卡死、不再吞话**

- 主模型流式输出加停滞守卫：上游长时间无输出时给出明确提示并回到聆听状态，不再整轮卡死（此前卡死后后续提问会被一并吞掉）。
- 打断意图判断失败时不再丢弃用户话（转为队列处理）；语音识别链路掉线自动重连，恢复后会话继续可用。
- 身份记忆加载从语音启动路径解耦，首问响应不再被阻塞。

**长内容写入不再静默失败**

- 语音里让助手把整段讨论写入一条长笔记时，单轮输出预算不够会导致工具调用参数被截断；此前系统会静默地按空参数执行并反复失败。现在被截断的调用不再执行，模型收到可行动的提示（改用分块写入）并自动提升单轮预算重试（1K → 4K，上限 8K）。

### 6.48 长会话新消息不再丢失；粘贴表格正常渲染（2026-09-20）

**长会话不再答非所问**

- 修复一个严重问题：会话消息超过 200 条后，最新发送的消息可能没有进入模型上下文——模型会接着旧话题继续工作（在死磕类长任务中表现为「埋头跑很久」），最终回答失败。现在上下文始终取最新消息，长会话与普通会话行为一致。
- 长会话的回答窗口同时收紧了「取最新」语义，重新生成、编辑重发路径不受影响。

**粘贴的表格正常显示**

- 从浏览器或表格软件复制粘贴的制表符表格，现在按真正的表格渲染（此前会塌成一大段连续文字）；代码块与缩进文本不会被误转。

### 6.47 审计与死磕完整性加固（2026-09-19 ~ 09-20）

**审计链路不再依赖截断**

- 工具输出不再静默截断：终端等长输出完整保留，超长时自动存档并给出可回读指针；审计核对时按存档回读全文，部分回读会显式标注「仅信封/预览可见」——看不到的证据不再被误判为编造。
- 长回答审计改为「分块声称清单」：整篇按段落分块提取关键声称与风险点（与问题无关、指代不可见内容等），逐条核对证据；未能覆盖的片段会明确告知用户「这部分未经二次核对」，并按低置信出货，不再假装全部核验过。
- 审计证据台账、判定上下文与低置信说明全部显式标注来源与覆盖范围；发送前有上下文上限硬守卫，宁可降级说明也绝不因超限跳过审计。

**死磕模式完成闸门**

- 「受限估计：待执行 / 待填写」这类占位内容不再能通过完成判定：验证器与裁判会看到交付物中的未执行占位并拒绝放行。
- 用户对已交付结果的批评或返工要求（如「写得很糟糕」「重写第三章」）会重新开启执行流程，不再被当成闲聊或仅做事实复核。
- 意图判断全面改由模型完成（新增「返工」意图），不再使用关键词清单。

**记忆纠正**

- 每条消息都由模型判断是否在纠正或否定长期记忆，不再依赖「不对 / 其实」等关键词命中；调用记账区分只读遥测与写路径，供成本治理使用。

### 6.46 工作区文件安全加固（2026-09-19）

**越权防护**

- 文件与文件夹的提供、目录列表、打包下载、Office 转换全部统一到同一套工作区路径校验：绝对路径越界、`..` 穿越、指向工作区外的符号链接、其他用户目录、非法字符（含空字节）一律拒绝；越狱式提示词无法绕过（提示词层同步声明边界）。
- 修复了一个下载接口可被符号链接绕过、读取工作区外文件的漏洞（独立评审发现的 Critical 级问题）；下载与打包改为「校验后以文件描述符读取」，堵住校验与读取之间的替换竞态。
- 附件交付从此只含工作区相对路径，界面不再出现系统绝对路径。

**本地转换与打包加固**

- 文档转换缓存增加内容校验与目录权限收紧；文件夹打包下载的压缩包内路径做跨平台清洗，避免在 Windows 侧解压时越界。
- 页面入口不再使用浏览器启发式缓存（避免部署后仍运行旧版本）；内容哈希资源改为长缓存，加载更快。

### 6.45 文件预览与 Office 完整预览；文件夹交付（2026-09-19）

**聊天里的文件预览**

- 上传的文件在发送后可直接点击预览：图片看大图、音视频内嵌播放、PDF / 文本 / Markdown / 代码按类型渲染；类型无法识别时显示工作区相对路径与下载按钮，不再暴露系统绝对路径。
- 修复了曾出现「初次打开预览空白、翻页才显示内容」与「预览窗口太矮无法浏览」的问题：文档查看器支持连续滚动、缩放与翻页。

**Office 文档预览**

- Word / Excel / PPT 点击即看：优先在服务端转换为 PDF 渲染，Excel 按工作表整页导出，表格与图表完整；转换组件不可用时自动改用纯浏览器渲染兜底，预览能力不缺失。自托管提示：安装 LibreOffice（core/calc/writer/impress）可获得服务端高保真预览效果。
- 新生成与历史（仅保存绝对路径）的附件卡片都能正常预览。

**文件夹交付与打包下载**

- 新增文件夹交付工具（provide_folder）：Agent 可以把整个文件夹作为卡片提供给用户；点开是文件夹管理器，可浏览子目录、预览与下载单个文件、一键下载整个文件夹的 zip 包（服务端打包）。

### 6.44 工作区编辑原语、快照回滚与持久进程（2026-09-19）

**工作区文件编辑**

- **Agent 可以直接改文件了**：新增工作区文件写入/精确编辑能力——修改已有文件时先读取、再做唯一匹配的字符串替换（多处匹配会拒绝并要求更精确的上下文），并返回 unified diff 供核对；新建或整体覆盖同样可用。编辑前若文件被外部改动（读取后被他人修改），系统会拒绝并提示重新读取，避免覆盖别人的修改。
- **误改可回滚**：每次写入/编辑前自动创建快照（影子 Git，保存在用户工作区之外，不污染文件列表）；可查看最近快照并一键恢复，恢复前还会自动为当前状态再存一次快照（恢复本身可再撤销）；快照之后新增的文件不会被删除。

**长任务与交互式命令**

- **持久进程会话**：新增 process 工具，可在受控环境下启动长期运行的命令（开发服务器、长时间测试/构建、需要持续输入的命令），随时写入输入、增量读取新输出、查看运行状态或终止；每个用户有并发上限与空闲自动回收，一次性命令仍走终端工具。

**安全加固**

- **权限确认不可被模型绕过**：修复了一个内部授权标志可被模型在工具参数中伪造的问题，涉及终端命令、快照恢复等需要确认的操作；授权现在只能由用户在确认流程中给出（高风险操作默认仍需用户授权）。

**稳定性**

- 进程会话在服务重启时自动清理，不残留孤儿进程；多字节（中文）输出在分段读取时不再被截断破坏。

### 6.43 身份回答边界与思考显示卫生（2026-09-18）

**回答质量**

- **记忆中的事实不再被判「凭空编造」**：修复了审计证据域只包含工具结果时、注入的长期记忆内容被判无据并触发拒答的问题——长期记忆现在作为权威来源进入核对（与记忆一致的陈述不再被要求重新检索举证）；当用户纠正或记忆与全局信息冲突时，以用户本轮消息与记忆为准。
- **自身信息如实作答**：涉及「你自己的仓库/源码/地址」等问题时，只依据该用户记忆中已有的内容回答；没有记忆就如实说明并可联网核实，不再凭印象编造。
- **思考过程不再泄漏内部规则**：深度思考中不再出现「根据系统提示词/内部规则/协调器注记」之类的元引用（提示词纪律 + 显示层确定性清洗双保险）。

### 6.42 移动端体验与批量导入加固（2026-09-18）

**客户端与导入**

- **移动端返回手势不再误关页面**：返回键/手势优先收起已打开的弹层与侧边栏，无层可收起时才触发页面返回，减少误退出。
- **会话批量导入**：支持一次导入多个对话文件；新导入通道对上传大小、解压资源与解析行数设上限，公式内容自动转义防注入，单文件失败自动回滚，导入过程流式展示。
- **CSV 大表不再错位**：修复表格单格 32k 字符硬限制导致的行列错位（超长单元格自动分片、无损拼接）。
- **平板与响应式布局修复**：平板断点下弹层与设置面板排版修正；窗口缩放/主题切换性能优化。

**外观与交互**

- **主题切换保留涟漪动画**：修复深色/浅色与皮肤切换时动画丢失的问题（切换期间抑制全站过渡、保留切换涟漪）。
- 首页「试试这些」引导卡移除；空态 Logo 区块在三套皮肤下局部垂直居中。

### 6.41 附件交付策略与质检提示静默（2026-09-18）

**交付与体验**

- **分析任务不再自动生成下载卡片**：此前终端/代码执行在工作区产生的中间文件会被自动收集为下载卡片（一次分析任务曾产生数百个卡片）。现在只有助手显式调用「提供文件」时才会生成下载卡片，中间产物与运行文件不再打扰用户。
- **质检说明不再出现在回答里**：软性复核提示（如「已自动二次复核」类说明）不再附加在回答末尾；审计发现真实矛盾或需要用户注意的红线情况仍会明确说明。

### 6.40 跨轮引用与质检假判词修复（2026-09-16）

**回答质量**

- **继续旧会话再问时不再错引编号**：此前在带引用回答的旧会话中继续提问并要求重新检索时，系统容易把上一轮回答中的来源细节与旧编号「对号入座」到本轮，被质检反复打回（极端情况反复数分钟后给出兜底稿）。现在：历史回答存在时，本轮首次检索结果会附带一条明确纪律——历史细节与编号不自动作数，须以本轮检索/浏览取得的证据为准。
- **质检「看不到对照表」假判不再空转**：质检在大上下文中偶发判「本轮无引用编号对照表」（实际对照表就在其上下文中），导致反复要求修正本来就合法的引用编号、真问题反而没人修。现在系统做确定性核对——编号全部合法而判词仅涉及对照表缺失时直接放行；判词中还有其他真问题时，自动剔除「修编号」的无效指令，只保留真问题打回。
- **「细节查无实据」时给出明确出路**：被要求删除无法核实的细节时，系统现在会提示可先用浏览器打开原文取得本轮证据后再引用，而非只能删除。
- 上述行为均有配置开关，可回退到旧行为。

### 6.39 超长上下文核对的透明度与覆盖加固（2026-09-16）

**回答质量**

- **兜底不再静默**：超长资料场景下，当逐条事实复核因核对服务繁忙/超时未能完成，或回答中仍有声称超出逐条复核的覆盖上限时，系统不再静默放行——回答末尾会附上一句说明（所引用数字与名称已确认存在于来源资料、建议对照原文），让本次回答的复核覆盖程度对你可见。该说明可配置关闭。
- **复核不到的声称自动换更宽证据窗口重试**：首轮逐条核对中「片段不足以判断」的声称，会自动携带更宽的原文片段再核对一次，减少因定位偏差造成的「查了等于没查」。
- 同时订正了 6.38 更新记录的措辞：现行主路径是「先逐条核对再放行」，确定性存在核对仅为核对机制不可用时的兜底。

### 6.38 审计收敛与超长上下文回答质量（2026-09-15）

**回答质量**

- **超长上下文不再反复生成**：当参考资料很长、超出质检（发送前审计）的可见范围时，回答中引用的具体数值此前会被反复判为「无法核对」并触发多轮重生成（极端情况十余分钟、最后给出一份带警示的兜底稿）。现在系统改为「先核对、再放行」：先按回答中的具体声称逐条定位完整资料中的相关片段，用独立的小型核对逐条验证（见下方「分段核对」）；与资料直接矛盾的声称会被点名打回。只有在核对机制本身不可用（服务异常/超时）时，才退化为确定性核对——引用的数字/名称确实存在于完整资料中即判定为「质检看不到」而非编造，放行且不再重生成。
- **同类意见连续出现两次即止损**：质检连续两次给出同一类意见时，系统不再继续重生成，直接进入兜底链（在被拒草稿中择优出货），避免长时间反复生成。
- **分段核对（新增）**：质检看不到证据时，系统按回答中的具体声称逐条定位相关资料片段，再用独立的小型核对逐条验证（每条声称只携带相关片段，互不干扰）；发现与资料直接矛盾的声称会点名打回，修正时只需修改点名部分。
- 上述行为均可通过配置关闭，回退到旧行为。

**子代理与长任务**

- 单次迭代 / 子代理运行上限由 10 分钟提升至 30 分钟，长时间研究型子任务不再被中途截断。

### 6.37 模型供应商扩展与视觉能力探测（2026-09-15）

**回答质量**

- 零引用守卫：回答使用了联网检索结果、最终稿却没有任何来源编号时，审计会打回一次并要求补标（只补标注、不重写内容），避免「有据不引」。
- 审计模型可独立配置：部署方可以为发送前审计单独指定模型（默认仍跟随助手所选模型）。

**模型供应商**

- 新增豆包（火山方舟）模型供应商，可在自定义助手中直接选择；支持思考开关与推理强度档位。
- 模型供应商收敛：Qwen3.8 系列的两个入口合并为 Qwen3.8(Local)——两者调用与回复结构一致；原 Flash-Next 入口下线，已选该模型的助手自动切换到 Qwen3.8(Local)。
- 系统设置 → 模型供应商：大语言模型按供应商分组（默认模型 / DeepSeek / MiMo / Qwen3.8(Local) / 豆包）分别覆写地址、密钥、模型名与采样参数；留空即沿用系统默认，各供应商互不影响。

**视觉能力**

- 未单独配置视觉模型时，系统先用一张测试图探测当前主模型是否具备视觉能力：具备则由主模型直接看图回答，不具备则明确提示「不具备视觉能力」，不再默默跳过或假装看过。
- 单独配置了视觉模型时仍优先使用独立视觉模型。

### 6.36 会话体验与渲染修复（2026-09-15）

**会话列表**

- **继续旧会话即置顶**：在侧栏打开一段过去的对话并发送消息后，该会话立即跳到所在分组（或「7天内 / 30天内 / 更早」分类）的最上方；刷新页面后位置保持一致，不再停留在原位。
- 会话排序口径统一为「分组顺序 → 最近一次用户消息时间」；单会话详情与列表返回一致的最近消息时间，时间分类更准确。

**Markdown 渲染**

- 修复「表格显示成一片竖线」：模型偶尔漏掉表格分隔行时，渲染阶段自动补全，历史消息无需重新生成即可正常显示为表格（代码块内的竖线不受影响）。

### 6.35 记忆系统与任务能力升级（2026-09-13 ~ 09-14）

**记忆**

- 记忆面板增强：概念分页与详情、单条遗忘、召回台账视图——每次召回了什么、来自哪里、权重多少都可追溯。
- 召回管线与评测升级：新增多跳证据分布、陷阱、压缩保持、记忆关闭对照四类评测剧本；概念合并、分组等高级能力默认关闭（影子评估），线上行为保持稳定。
- 后台任务可携带相关记忆（默认关闭、按需开启）。
- 旧记忆通道退休：新记忆架构启用时，旧的「每日摘要 / 梦境」不再生成（历史数据保留可读）；文件记忆继续活跃并作为新架构的输入。

**任务与工作台**

- 后台任务支持「插话」引导：运行中的任务可以接收新指令并调整方向。
- 长会话消息分页加载；历史会话支持只读检索。
- 子任务产物回流：子代理生成的文件以下载卡片返回。
- 上传增强：上传前的预览与取消。
- 工作台笔记实时字数统计。

**语音**

- 填充词首字门控：回答开口前的口头填充不再每轮播报，只在需要衔接时出现，首字响应更快。

**部署**

- 新增一键 Docker 部署（应用 + 数据库，数据卷持久化）与完整使用手册；仓库内附「AI 助手代部署」技能，可让 AI 助手按手册完成部署与冒烟验证。

### 6.34 用户信息与用户级模型配置（2026-09-12 ~ 09-13）

- 用户信息：可设置昵称与头像（仅影响本人账号展示）。
- 用户级模型供应商覆盖：可为当前用户单独配置各类型模型（对话 / 视觉 / 语音 / 嵌入等）的供应商、地址与密钥；自定义地址不会回落系统密钥。
- 语音识别面板：识别热词与通用识别密钥集中到系统设置。

### 6.33 回答质量与引用体系加固（2026-09-09 ~ 09-12）

- 引用编号污染修复：跨轮对话不再复用上一轮的编号，历史引用在拼接上下文时被中和，避免「编号对不上」。
- 审计上下文新增引用编号对照表，消除两套编号并存导致的合法引用误判与反复重答。
- 新实体检索触发：遇到未见过的新产品 / 新事件时先检索再回答，不再凭记忆断言不存在。
- 零引用标记：草稿大量使用检索结果却没有任何引用时，审计器会标记并要求补充。
- 数值溯源：回答中的关键数值需能追溯到证据来源。
- 搜索发布时间优先级：检索结果按发布时间优先排序，时效性问题回答更准。
- 移动端修复：文本溢出、拖拽、剪贴板混合内容、公式计数等细节收口。

### 6.32 语音体验与模型扩展（2026-09-06 ~ 09-08）

- 语音对话每一轮携带本地时间戳，时间相关的问题回答更准确。
- 新增 Qwen3.8-Flash-Next 模型供应商支持。
- 思考 / 非思考采样参数按模型自动匹配，长输出保持不截断。

### 6.31 文档与记录更新（2026-09-06）

**产品文档**

- 自述文件（README）全面重写：功能数量、服务模块数、测试规模等量化信息与当前代码重新核对校准；新增系统模块全景图、模型网关等文档入口；产品介绍页与皮肤版本的指引保持不变。
- 系统文档更名为「changelog.md」：原「系统功能文档 func.md」正式更名为更新记录文件，功能介绍与版本记录内容合并保留；Agent 在被问到产品功能、自身能力、版本更新时读取的即是这份文件。

**记录方式**

- 版本记录从「只保留最近一次增量」改为累积保留：每次更新新增一节，历史不再被覆盖，更早的变化也能回查。

### 6.30 最近一次更新（产品视角增量）

**模型与推理**

- 私有化大模型接入：新增 Qwen3.8（vLLM）模型类型，可在自定义助手中直接配置 API 地址/密钥/模型名（留空自动回退服务器配置）；思考/非思考双采样参数组自动切换。
- 深度思考档位选择：DeepSeek 支持 Max/High/Low/关闭，Qwen3.8 VLLM 支持最强/均衡/低/关闭，可在输入框直接切换。
- 助手模型全局继承：主对话、质检、上下文压缩、裁决等子任务全部跟随助手所选模型，不再混用全局默认。
- 长输出不再截断：默认不设输出上限，回复长度由模型最大输出决定。

**语音体验**

- 更自然的语音节奏：回答开口前有填充词（如"让我想想"）、聆听中适时应和（"嗯"），与主语音同队列串行，打断更跟手。
- 语音工具箱扩充：语音对话中可调用历史会话检索与 MCP 服务器工具，工具名中英文映射。

**回答质量与防编造**

- 质检全程静默：审核中间步骤不再闪现给用户；被拒草稿按「复制编辑修正协议」仅局部修改，相似度门禁拦截全稿重写。
- 已定裁决不再翻案：同轮驳回结论进入裁决台账，审计器不再反复摇摆；预算耗尽后从备选草稿择优兜底。

**稳定性与体验**

- 心跳保活：上游流停滞时自动唤醒超时检查，长生成不再静默挂死。
- 皮肤系统上线：三套内置皮肤（青野平面/墨韵纸间/黑白构成）× 明暗双模式，令牌化覆盖登录页、设置、笔记、工作台、语音等全部界面；支持上传自定义 CSS 皮肤，上传即应用；深色模式全表面对比度修复。
- 笔记粘贴增强：Word/Google Docs/飞书粘贴时自动内联样式并清洗危险标签，字号、字体族、字距、大小写等排版完整保留。
- 侧栏快捷菜单：笔记侧栏的笔记本与笔记支持呼出菜单（重命名/移动/删除），工作台笔记本同样支持。
- 输入区体验：光标跟随（长输入自动滚动到光标处）、定高容器内部滚动、移动端高度自适应。

**联网检索**

- 联网搜索升级为 Exa 优先 + Bocha/Firecrawl/Tavily 自动回退链，结果持久化可追溯。
