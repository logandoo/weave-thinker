...
- **超长上下文不再反复生成**：当参考资料很长、超出质检（发送前审计）的可见范围时，回答中引用的具体数值此前会被反复判为「无法核对」并触发多轮重生成（极端情况十余分钟、最后给出一份带警示的兜底稿）。现在系统会做确定性核对——引用的数字/名称若确实存在于完整资料中，即判定为「质检看不到」而非编造，直接放行，不再重生成。
