...
## 用法
通过 `execute_code` 调用 `openpyxl`（预装，版本 3.1.5；`pandas` 3.x 也已预装）生成代码并执行，输出 `.xlsx` 文件保存到**工作区根目录**（用绝对路径；需要给用户下载时调用 `provide_file` 生成下载卡片，写文件本身不会自动出卡片）。
...
## 规则
1. 最终交付文件必须写入**工作区根目录**（绝对路径），并调用 `provide_file` 生成下载卡片；草稿/中间数据写 scratch 临时目录即可。
