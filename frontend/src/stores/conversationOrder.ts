...
/** 服务端「最近用户消息」时间（仅 last_user_message_at，不用 updated_at 回退）
 *  ——追平判定的唯一权威输入；缺失/非法 = 0。 */
...
/** 服务端快照是否已包含本次发送的用户消息（严格晚于提升前值）——纯服务端
 *  时间戳互比，客户端时钟偏差不影响判定（A4.9 Minor-2）。 */
 ...
 * 服务端提交之前生成、在本地置顶之后才到达：只要服务端快照的最近用户消息仍
